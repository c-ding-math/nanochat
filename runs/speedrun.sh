...
torchrun --standalone --nproc_per_node=$NPROC_PER_NODE -m scripts.base_train --  --depth=4 --max-seq-len=128 --device-batch-size=1 --eval-tokens=128 --core-metric-every=-1 --total-batch-size=1024 --num-iterations=4 --target-param-data-ratio=20 --run=$WANDB_RUN
