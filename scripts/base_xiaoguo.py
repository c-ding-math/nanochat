...
def main():
    ...
    while i < 1000 :
        ...
        print0(f"Token ID: {i} | String: '{token_str}' | Bytes: {len(token_str.encode('utf-8'))}")
    ...
    if 'sample' in eval_modes and not is_hf_model:
        ...
        print0("="*80)
        if ddp_rank == 0:
            # /data/dingchun/dataset/processed-declarations/Mathlib.Algebra.AddTorsor.Basic.txt
            # /data/dingchun/dataset/processed-declarations/Mathlib.Algebra.Ring.Subring.Pointwise.txt
            # /data/dingchun/dataset/processed-declarations/Mathlib.Algebra.Tropical.Lattice.txt
            with open('/data/dingchun/dataset/processed-declarations/Mathlib.Algebra.Tropical.Lattice.txt', 'r', encoding='utf-8') as file:
                
                position = 350000
                lines = file.readlines()
                print0(f"Total lines in file: {len(lines)}")
                content = "".join(lines[position:position+1000])
                printContent = lines[position:position+1010]
                printContent = "".join(printContent)
                print0(printContent)
                print0("&" * 80)
            
            prompts = [
                content,  # Use the first 1024 characters as a prompt
            ]
            ...
            for prompt in prompts:
                ...
                print0(tokenizer.decode(sample[0][len(tokens):]))  # Print only the generated continuation


            print0("\nUnconditioned samples:")
            tokens = tokenizer("", prepend="<|bos|>")
            with autocast_ctx:
                uncond, _ = engine.generate_batch(tokens, num_samples=8, max_tokens=128, temperature=0.01)
